...
function sumK(arr, k) {
  return arr.reduce((a, r) => a + (Number(r[k]) || 0), 0);
}
...
function buildTimeSeries(rows, period) {
  ...
  rows.forEach(r => {
    const key =
      period === "day" ? (r.date || "").slice(0, 10) :
      period === "week" ? (r.Week || "").slice(0, 10) :
      (r.Month || "").slice(0, 7);
...
async function loadData() {
  ...
  for (const url of sources) {
    try {
      ...
      console.error("loadData failed for", url, err);
    }
  }
...
function DonutChart({ data, title, fmtVal, isPhone }) {
  ...
  const slices = data.map(d => {
    ...
    const dl = Math.max(0, circ * pct - gap);
  ...
  return (
    <div style={{ background: "#fff", borderRadius: 12, padding: isPhone ? 12 : 16, boxShadow: "0 1px 4px rgba(0,0,0,0.06)", border: "1px solid #f1f5f9" }}>
      <div style={{ fontSize: 11, fontWeight: 600, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 12 }}>
        {title}
      </div>

      <div style={{ display: "flex", alignItems: "center", gap: 16, flexDirection: isPhone ? "column" : "row" }}>
    ...
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", height: "auto" }}>
      ...
      {chartType === "line" ? (
        <g>
          ...
          {vals.map((v, i) =>
            i % step === 0 ? (
              <text key={i} x={xP(i)} y={H - 4} textAnchor="middle" fontSize="9" fill="#9ca3af">
              ...
              <g key={i}>
                ...
                {isProj && (
                  <g>
                    ...
                    <text x={x} y={y - 9} textAnchor="middle" fontSize="9" fill="#fff" fontWeight="700">
                      {"▲ " + fmtY(v)}
                    </text>
                  </g>
                )}
...
export default function Dashboard() {
  ...
  const markets = useMemo(() => {
    const s = {};
    rawData.forEach(r => { if (r.market) s[r.market] = 1; });
    return Object.keys(s).sort();
  }, [rawData]);

  const chanCats = useMemo(() => {
    const s = {};
    rawData.forEach(r => { if (r.cat) s[r.cat] = 1; });
    return Object.keys(s).sort();
  }, [rawData]);
  ...
  const donutData = useMemo(() => {
    ...
    filtered.forEach(r => {
      ...
      cats[r.cat].leads += r.leads;
      cats[r.cat].booked += r.booked;
      cats[r.cat].revenue += r.revenue;
      cats[r.cat].completed += r.completed;
    });
    return Object.keys(cats).sort().map(k => ({
      label: k,
      ...cats[k],
...
`;
  ...
  const pacingBanner = pacing && (period === "week" || period === "month") ? (
    <div style={{ marginBottom: 16, padding: isPhone ? "10px 12px" : "10px 16px", borderRadius: 10, background: "#eff6ff", border: "1px solid #bfdbfe", display: "flex", alignItems: isPhone ? "flex-start" : "center", gap: 10, flexWrap: "wrap", flexDirection: isPhone ? "column" : "row" }}>
      <span style={{ fontSize: 16 }}>📈</span>
      <div>
        <span style={{ fontSize: 12, fontWeight: 700, color: "#1d4ed8" }}>
          {period === "week" ? "Week" : "Month"} Pacing · {pacing.label} · {(pacing.pct * 100).toFixed(0)}% elapsed
        </span>
        <span style={{ fontSize: 11, color: "#3b82f6", marginLeft: isPhone ? 0 : 8, display: isPhone ? "block" : "inline" }}>
          Projected values shown on each metric card
        </span>
      </div>
      <div style={{ marginLeft: isPhone ? 0 : "auto", background: "#dbeafe", borderRadius: 20, padding: "2px 10px" }}>
    ...
    <div style={containerStyle}>
      <div style={innerStyle}>
        <div style={{ marginBottom: 20 }}>
          <h1 style={{ margin: 0, fontSize: isPhone ? 18 : 20, fontWeight: 700, color: "#111827" }}>KPI Dashboard</h1>
          <p style={{ margin: "3px 0 0", fontSize: isPhone ? 11 : 12, color: "#6b7280" }}>
            NuBrakes · {market} · {chanCat} · {tab === "overview" ? overviewLabel : periodLabel}
          </p>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 6, flexWrap: "wrap" }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "3px 10px", borderRadius: 20, background: usingFallback ? "#fff7ed" : "#ecfdf5", border: "1px solid " + (usingFallback ? "#fed7aa" : "#6ee7b7") }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 600, color: usingFallback ? "#c2410c" : "#065f46" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 18, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <select value={chanCat} onChange={e => setChanCat(e.target.value)} style={selectStyle}>
            <option>All Channels</option>
            {chanCats.map(c => <option key={c}>{c}</option>)}
          </select>
        </div>

        <div style={{ display: "flex", marginBottom: 22, borderBottom: "1.5px solid #e5e7eb", overflowX: "auto" }}>
        ...
        {tab === "overview" && (
          <div>
            ...
            <div style={{
              display: "grid",
              gridTemplateColumns: isPhone ? "1fr" : isTablet ? "repeat(2, minmax(0, 1fr))" : "repeat(auto-fill,minmax(190px,1fr))",
              gap: 12,
              marginBottom: 24
            }}>
              {METRICS.map(m => {
                ...
                return (
                  <div key={m.key} style={{ ...baseCardStyle, padding: isPhone ? "12px" : "16px 18px 14px" }}>
                    <div style={{ fontSize: 10, fontWeight: 600, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 5 }}>
                      {m.label}
                    </div>
                    <div style={{ fontSize: isPhone ? 22 : 24, fontWeight: 700, color: "#111827", marginBottom: 8 }}>
                      {m.fmt(c)}
                    </div>

                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 8 }}>
                      <span style={{ fontSize: 11, fontWeight: 600, color: good ? "#10b981" : "#f43f5e", background: good ? "#ecfdf5" : "#fff1f2", padding: "2px 7px", borderRadius: 20, whiteSpace: "nowrap" }}>
            ...
            <div style={{
              display: "grid",
              gridTemplateColumns: isPhone ? "1fr" : isTablet ? "repeat(2, minmax(0, 1fr))" : "repeat(auto-fill,minmax(280px,1fr))",
              gap: 12
            }}>
              <DonutChart data={donutData.map(d => ({ label: d.label, value: d.leads, color: d.color }))} title="Leads by Channel Category" isPhone={isPhone} />
              <DonutChart data={donutData.map(d => ({ label: d.label, value: d.booked, color: d.color }))} title="Bookings by Channel Category" isPhone={isPhone} />
              <DonutChart data={donutData.map(d => ({ label: d.label, value: d.revenue, color: d.color }))} title="Revenue by Channel Category" fmtVal={v => "$" + Math.round(v).toLocaleString()} isPhone={isPhone} />
              <DonutChart data={donutData.map(d => ({ label: d.label, value: d.completed, color: d.color }))} title="Completed Jobs by Channel Category" isPhone={isPhone} />
            </div>
          </div>
        )}
        ...
        {tab === "trends" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: isPhone ? "stretch" : "center", flexDirection: isPhone ? "column" : "row", gap: 12, marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", gap: 4, background: "#f1f5f9", borderRadius: 8, padding: 3, alignSelf: isPhone ? "flex-start" : "auto" }}>
            ...
            <div style={{ ...baseCardStyle, marginBottom: 12, padding: isPhone ? "12px" : "16px 18px 14px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: isPhone ? "repeat(2, minmax(0, 1fr))" : "repeat(auto-fill,minmax(140px,1fr))", gap: 10 }}>
              {["Peak", "Average", "Latest", "Projected"].map((lbl, li) => {
                ...
                return (
                  <div key={lbl} style={{ ...baseCardStyle, borderTop: li === 3 ? "2px solid #3b82f6" : "", padding: isPhone ? "12px" : "16px 18px 14px" }}>
                    ...
                    <div style={{ fontSize: isPhone ? 18 : 20, fontWeight: 700, color: li === 3 ? "#3b82f6" : selMetric.color }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16, maxHeight: 360, overflowY: "auto", padding: "4px 0" }}>
                {chatHistory.map((msg, i) => {
                  ...
                  return (
                    <div key={i} style={{ display: "flex", justifyContent: isUser ? "flex-end" : "flex-start" }}>
                      <div style={{ maxWidth: isPhone ? "92%" : "80%", padding: "10px 14px", borderRadius: 12, fontSize: 13, lineHeight: 1.7, whiteSpace: "pre-wrap", background: isUser ? "#6366f1" : "#f8fafc", color: isUser ? "#fff" : "#374151", borderBottomRightRadius: isUser ? 2 : 12, borderBottomLeftRadius: isUser ? 12 : 2, boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
                        {msg.text}
                      </div>
                    </div>
                  );
                ...
                {aiLoading && (
                  <div style={{ display: "flex", justifyContent: "flex-start" }}>
                    <div style={{ padding: "10px 14px", borderRadius: 12, background: "#f8fafc", fontSize: 13, color: "#9ca3af" }}>
                      Thinking...
                    </div>
                  </div>
                )}
            ...
            <div style={{ display: "flex", gap: 8, flexDirection: isPhone ? "column" : "row" }}>
              ...
              <button
                ...
                style={{
                  ...
                  fontWeight: 600,
                  cursor: "pointer",
                  opacity: (aiLoading || !userQuestion.trim()) ? 0.5 : 1,
                  width: isPhone ? "100%" : "auto"
                }}
              >
                Ask
